<template>
  <section class="w-100">
    ...
    <v-dialog v-model="dialog" persistent max-width="350px">
      <v-card>
        <ModalDetailHeader header="Yeni Ünvan" />
...
export default {
  ...
  head() {
    return { title: `Ünvanlar ${process.env.title}` };
  },
  components: {
    TableHeader,
    ModalBtnDropdown,
  ...
  async asyncData(state) {
    await state.store.dispatch("titles/getAll");
  },
  computed: {
    ...mapGetters({
      entities: "titles/getAll",
    }),
  },
  methods: {
    clearInput(){
    ...
    async Insert() {
      ...
      await this.insertAction("titles", this.entity);
      await this.$store.dispatch("titles/getAll");
      this.dialog = false;
    },
   async edit(id){
      this.loading = true;
      this.dialogUpdate = true;
      this.updateEntity = await this.$TitleService.Get(id);
      this.loading = false;
    },
    async Update(){
      if (!this.$refs.form.validate()) return;
      await this.updateAction("titles", this.updateEntity);
      await this.$store.dispatch("titles/getAll");
